...
# グローバルモード以外は実行しない
if [ "$IPAK_PURGE_MODE" != "global" ]; then
    echo "Error: Only global purge mode is supported"
    exit 1
fi
...
# 設定ファイルも削除
if [ -d "data/etc" ]; then
    find data/etc -type f | while read -r file; do
        # dataディレクトリからの相対パスを取得
        rel_path="${file#data/}"
        # インストール先のファイルを削除
        target_file="$DATA_DIR/$rel_path"
...
if [ -f "control/postrm" ]; then
    sh ./control/postrm purge
fi

echo "Purge of $IPAK_PROJECT_NAME $IPAK_PROJECT_VERSION completed successfully."
