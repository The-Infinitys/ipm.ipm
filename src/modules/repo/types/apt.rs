...
use ipak::modules::pkg::{
    AboutData, AuthorAboutData, PackageAboutData, PackageData,
    PackageRange, PackageVersion, RelationData,
};
                        ...
                        for arch in &self.architectures {
                            ...
                            for para in deb822.paragraphs() {
                                ...
                                match Package::try_from_deb822_paragraph(
                                    ...
                                    Err(e) => eprintln!(
                                        "Warning: Skipping package due to parsing error: {}",
                                        e
                                    ), // パースエラーはスキップ
                                }
                            }
                    ...
                    for component in &self.components {
                        ...
                        for para in deb822.paragraphs() {
                            ...
                            match Package::try_from_deb822_paragraph(
                                ...
                                Ok(package) => packages.push(package),
                                Err(e) => eprintln!(
                                    "Warning: Skipping source package due to parsing error: {}",
                                    e
                                ),
                            }
                        }
...
impl Package {
            ...
            .map(|group_str| {
                group_str
                    ...
                    .map(|dep_part| {
                        ...
                        let parts: Vec<&str> =
                            trimmed_dep.splitn(2, ' ').collect();
                        let name = parts[0].to_string();
                        let range = if parts.len() > 1 {
                            // Version range exists, e.g., "(>= 1.0)"
                            let version_range_str = parts[1]
                                .trim_matches(|c| c == '(' || c == ')');
                            VersionRange::from_str(version_range_str)
                                .unwrap_or_default()
                        } else {
    ...
    fn parse_provides(provides_str: &str) -> Vec<PackageVersion> {
        provides_str
            ...
            .map(|provide_part| {
                ...
                let parts: Vec<&str> =
                    trimmed_provide.splitn(2, ' ').collect();
                let name = parts[0].to_string();
    ...
    pub fn try_from_deb822_paragraph(
        ...
        let package = fields.remove("Package").ok_or_else(|| {
            "Missing mandatory 'Package' field".to_string()
        })?;
        let version = fields.remove("Version").ok_or_else(|| {
            "Missing mandatory 'Version' field".to_string()
        })?;
        let maintainer = fields.remove("Maintainer").ok_or_else(|| {
            "Missing mandatory 'Maintainer' field".to_string()
        })?;
        ...
        let depends = fields
            .remove("Depends")
            .map_or_else(Vec::new, |s| Self::parse_dependencies(&s));
        ...
        let suggests = fields
            ...
            .map_or_else(Vec::new, |s| Self::parse_dependencies(&s));
        let conflicts =
            fields.remove("Conflicts").map_or_else(Vec::new, |s| {
                Self::parse_dependencies(&s)
                    .into_iter()
                    .flatten()
                    .collect()
            });
        let replaces =
            fields.remove("Replaces").map_or_else(Vec::new, |s| {
                Self::parse_dependencies(&s)
                    .into_iter()
                    .flatten()
                    .collect()
        ...
        let provides = fields
            .remove("Provides")
            .map_or_else(Vec::new, |s| Self::parse_provides(&s));
...
impl fmt::Display for Package {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        writeln!(
            ...
            self.architecture
                .as_ref()
                .map_or("N/A".to_string(), |s| s.blue().to_string()),
        ...
        if let Some(description) = &self.description {
            writeln!(
                f,
                "Description: {}",
                description.truecolor(255, 165, 0)
        ...
        if !self.depends.is_empty() {
            ...
            for group in &self.depends {
                let alts: Vec<String> =
                    group.iter().map(|pr| format!("{}", pr)).collect();
...
impl From<Package> for PackageData {
    fn from(pkg: Package) -> Self {
        let (author_name, author_email) =
            parse_maintainer(&pkg.maintainer);
        ...
        //         eprintln!(
        //             "Debug: Description '{}' is not directly mapped to PackageData.",
        //             desc
        //         );
        //     }
        //     if let Some(section) = pkg.section {
        //         eprintln!(
        //             "Debug: Section '{}' is not directly mapped to PackageData.",
        //             section
        //         );
        //     }
        //     // 他のフィールドも同様
    ...
    if let Some(start_paren) = maintainer_str.rfind('<') {
        if let Some(end_paren) = maintainer_str.rfind('>') {
            if end_paren > start_paren {
                let name =
                    maintainer_str[..start_paren].trim().to_string();
                let email = maintainer_str[start_paren + 1..end_paren]
                    .trim()
                    .to_string();
                return (name, email);
            }
...
async fn fetch_url(
    ...
    if url.ends_with(".gz") {
        ...
        decoder.read_to_string(&mut content).map_err(|e| {
            io::Error::new(
                io::ErrorKind::InvalidData,
                format!("Failed to decompress gzip content: {}", e),
            )
        })?;
        Ok(content)
    } else {
...
mod tests {
    ...
    async fn test_package_to_packagedata_conversion() {
        ...
        let debian_pkg = Package {
            ...
            description: Some(
                "A sample package for testing conversion.".to_string(),
            ),
            ...
            depends: Package::parse_dependencies(
                "dep-a (>= 1.0) | dep-b (= 2.0), dep-c",
            ),
            recommends: Package::parse_dependencies("rec-x"),
            suggests: Package::parse_dependencies("sugg-y (>> 3.0)"),
            conflicts: Package::parse_dependencies(
                "conflict-old (<< 1.0)",
            )
            .into_iter()
            .flatten()
            .collect(),
            replaces: Package::parse_dependencies("old-replaced-pkg")
                .into_iter()
                .flatten()
                .collect(),
            provides: Package::parse_provides(
                "virtual-pkg (= 4.0), another-virtual",
            ),
            built_using: None,
        ...
        assert_eq!(
            ipak_pkg_data.about.package.version.to_string(),
            "1.2.3-1ubuntu1"
        ); // Versionの文字列比較
        assert_eq!(ipak_pkg_data.about.author.name, "John Doe");
        assert_eq!(
            ipak_pkg_data.about.author.email,
        ...
        println!("{}", ipak_pkg_data);
        assert_eq!(ipak_pkg_data.relation.depend[0][0].name, "dep-a");
        assert_eq!(
            ipak_pkg_data.relation.depend[0][0].range.to_string(),
            ">= 1.0"
        );
        assert_eq!(ipak_pkg_data.relation.depend[0][1].name, "dep-b");
        assert_eq!(
            ipak_pkg_data.relation.depend[0][1].range.to_string(),
            "= 2.0"
        );
        assert_eq!(ipak_pkg_data.relation.depend[1][0].name, "dep-c");
        assert_eq!(
            ipak_pkg_data.relation.depend[1][0].range.to_string(),
            "*"
        ); // dep-c にはバージョン範囲がない
        ...
        let conflict_names: Vec<String> = ipak_pkg_data
            .relation
            .conflicts
            .iter()
        ...
        assert_eq!(
            ipak_pkg_data.relation.virtuals[0].version.to_string(),
            "= 4.0"
        );
